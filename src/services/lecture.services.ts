...
@Injectable()
export class LectureService extends PrismaClient {
  // 강의 생성
  async createLecture(dto: CreateLectureDto): Promise<CreateLectureDto | null> {
    try {
  ...
  // 특정 ID의 강의 조회
  async getLectureById(id: string): Promise<GetLectureDto | null> {
    try {
  ...
  // 강의 삭제
  async deleteLecture(id: string): Promise<GetLectureDto | null> {
    try {
      const lecture = this.getLectureById(id);
