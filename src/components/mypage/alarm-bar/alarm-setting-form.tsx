...
export default function AlarmSettingForm({ item }: { item?: any }) {
  const form = useForm<z.infer<typeof AlarmSettingSchema>>({
    resolver: zodResolver(AlarmSettingSchema),
    defaultValues: {
      isNewNews_SMS: item?.isNewNews_SMS ?? true,
      isNotice_SMS: item?.isNotice_SMS ?? true,
      isPromotion_SMS: item?.isPromotion_SMS ?? true,
      isNewNews_Email: item?.isNewNews_Email ?? true,
      isNotice_Email: item?.isNotice_Email ?? true,
      isPromotion_Email: item?.isPromotion_Email ?? true,
    },
  });
